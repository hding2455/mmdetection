...
def bbox2delta(proposals, gt, means=(0., 0., 0., 0.), stds=(1., 1., 1., 1.)):
    ...
    We usually compute the deltas of x1, y1, x2, y2 of proposals w.r.t ground
    ...
    Returns:
        Tensor: deltas with shape (N, 4), where columns represent dx1, dy1,
            dx2, dy2.
    """
...
def delta2bbox(rois,
    ...
    px = ((rois[:, 0] + rois[:, 2]) * 0.5).unsqueeze(1).expand_as(dx1)
    py = ((rois[:, 1] + rois[:, 3]) * 0.5).unsqueeze(1).expand_as(dy1)
    ...
    bboxes_size = list(deltas.shape)
    bboxes_size[1] = int(bboxes_size[1]/2)
    bboxes = torch.stack([x1, y1, x2, y2], dim=-1).view(bboxes_size)
    return bboxes
